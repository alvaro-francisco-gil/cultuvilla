...
import { createBarrio } from '@cultuvilla/shared/services/municipalityService';
...
describe('<BarriosManager>', () => {
  it('any member submitting the form creates the barrio directly (optimistic)', async () => {
    ...
    await waitFor(() =>
      expect(createBarrio).toHaveBeenCalledWith('m1', {
  ...
  it('an admin creates the barrio the same way', async () => {
    ...
    await waitFor(() =>
      expect(createBarrio).toHaveBeenCalledWith('m1', {
        name: 'Norte', municipalityId: 'm1', proposedBy: 'boss',
      }),
    );
  });
});
