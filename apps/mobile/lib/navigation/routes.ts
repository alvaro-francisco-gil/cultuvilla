...
export function myVillageHref(villageId?: string): Href {
  return villageId ? href(`/(tabs)/mi-pueblo?villageId=${encodeURIComponent(villageId)}`) : routes.myVillage;
}

export const villageHref = (villageSlug: string): Href => href(villagePath(villageSlug));

export const villageSectionHref = (villageSlug: string, section: VillageSection, query?: string): Href =>
  href(`${villageSectionPath(villageSlug, section)}${query ? `?${query}` : ''}`);

export const eventHref = (event: Parameters<typeof eventLinkTarget>[0]): Href =>
  href(entityPath('event', eventLinkTarget(event)));

export const seatClaimHref = (event: Parameters<typeof eventLinkTarget>[0], token: string): Href =>
  href(seatClaimPath(eventLinkTarget(event), token));

export const newsHref = (post: { id: string; title: string; villageSlug: string }): Href =>
  href(entityPath('news', post));

export const orgHref = (org: { id: string; name: string; villageSlug: string }, query?: string): Href =>
  href(`${entityPath('organization', orgTarget(org))}${query ? `?${query}` : ''}`);

export const orgEditHref = (org: { id: string; name: string; villageSlug: string }): Href =>
  href(entityEditPath('organization', orgTarget(org)));

export const orgJoinHref = (org: { id: string; name: string; villageSlug: string }): Href =>
  href(orgJoinPath(orgTarget(org)));

export const placeHref = (villageSlug: string, place: { id: string; name: string }): Href =>
  href(entityPath('place', { id: place.id, title: place.name, villageSlug }));

export const placeEditHref = (villageSlug: string, place: { id: string; name: string }): Href =>
  href(entityEditPath('place', { id: place.id, title: place.name, villageSlug }));

export const barrioHref = (villageSlug: string, barrio: { id: string; name: string }): Href =>
  href(entityPath('barrio', { id: barrio.id, title: barrio.name, villageSlug }));

export const barrioEditHref = (villageSlug: string, barrio: { id: string; name: string }): Href =>
  href(entityEditPath('barrio', { id: barrio.id, title: barrio.name, villageSlug }));

export const festivalPosterHref = (poster: Parameters<typeof festivalPosterLinkTarget>[0]): Href =>
  href(entityPath('festivalPoster', festivalPosterLinkTarget(poster)));

export const festivalPosterEditHref = (poster: Parameters<typeof festivalPosterLinkTarget>[0]): Href =>
  href(entityEditPath('festivalPoster', festivalPosterLinkTarget(poster)));

export const wordHref = (villageSlug: string, termSlug: string): Href => href(wordPath(villageSlug, termSlug));

export const newWordHref = (villageSlug: string, query?: string): Href =>
  href(`${newWordPath(villageSlug)}${query ? `?${query}` : ''}`);

export const personHref = (personId: string, query?: string): Href =>
  href(`${personPath(personId)}${query ? `?${query}` : ''}`);

export const userHref = (uid: string): Href => href(userPath(uid));

/** Create-or-edit screens take their subject as query params, not a path. */
export function createEventHref(params: { villageId?: string; eventId?: string } = {}): Href {
  return href(`/crear/evento${toQuery(params)}`);
}

export function createNewsHref(params: { villageId?: string; newsId?: string } = {}): Href {
  return href(`/crear/noticia${toQuery(params)}`);
}

export const discoverOrganizeHref = (municipalityId: string): Href =>
  href(`/descubrir/organizar/${municipalityId}`);

export const discoverStartHref = (municipalityId: string): Href =>
