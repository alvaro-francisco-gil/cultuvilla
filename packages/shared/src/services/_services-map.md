...
| [personService](personService.ts) | `persons/` | "Persona" profiles (proxy profiles, up to 50 per user) for family-member sign-ups. CRUD. | `getPerson`, `getPersonsByCreator`, `getPersonByUserId`, `createPerson`, `updatePerson`, `deletePerson`, `getPersonsByBarrio`, `getPersonsByBurialPlace` |
...
| [villageMemberService](villageMemberService.ts) | `municipalities/{mid}/members/` + collection group | Village membership: add/remove, role checks, listing a user's villages. `joinVillage` is the self-service join entry point — it calls `addVillageMember` then `userService.setActiveMunicipality` so the joined village becomes the user's active village. | `getVillageMember`, `getVillageMembers`, `addVillageMember`, `joinVillage`, `removeVillageMember`, `isVillageMember`, `isVillageAdmin`, `getUserMemberships` |
