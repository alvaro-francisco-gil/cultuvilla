 ...
 *
 * One-off: the entity-comments feature added REQUIRED `commentCount` /
 * `reactionCounts` fields to the 5 comment-capable entities (event,
 * organization, festivalPoster, place, barrio). Existing dev docs predate
 * those fields, so the strict Zod converter now throws on read. Backfill
 * every doc missing either field with the same zero defaults the model
 * factories use (`commentCount: 0`, `reactionCounts: { like: 0, heart: 0 }`).
 *
 * Collections:
 *   - top-level: events, organizations, festivalPosters
 *   - nested: for every municipalities/{id}, its places and barrios subcollections
 *
 * `news` already has these fields (NewsPostDataModel) — not touched here.
 *
 * USAGE
 *   node scripts/backfill-entity-comment-counts.mjs
 *
 * Idempotent: only patches docs where `commentCount` or `reactionCounts` is
 * missing; re-running after a full backfill patches 0 docs.
 */
...
const DEFAULT_REACTION_COUNTS = { like: 0, heart: 0 };

function needsPatch(data) {
  return data.commentCount === undefined || data.reactionCounts === undefined;
}

function patchFor(data) {
  const patch = {};
  if (data.commentCount === undefined) patch.commentCount = 0;
  if (data.reactionCounts === undefined) patch.reactionCounts = DEFAULT_REACTION_COUNTS;
  return patch;
}
...
async function main() {
  console.log(`Backfilling commentCount/reactionCounts against ${PROJECT_ID}\n`);
