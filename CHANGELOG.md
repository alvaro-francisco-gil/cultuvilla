...
### Added
- **In-app photo cropper for square images (avatars, personas, escudos, community settings).** Picking any square image now opens a pan/zoom cropper so you can trim the exact 1:1 section you want — even from an already-square source, which the OS editor behind `expo-image-picker`'s `allowsEditing` can't do on Android and doesn't offer at all on web. Implemented as a platform split behind the unchanged `pickImageAsBlob({ square: true })`: native uses `react-native-image-crop-picker` (uCrop / TOCropViewController), web uses `react-easy-crop` in a root-mounted `<CropperHost/>` overlay ([apps/mobile/lib/imageCrop.native.ts](apps/mobile/lib/imageCrop.native.ts) / [apps/mobile/lib/imageCrop.web.tsx](apps/mobile/lib/imageCrop.web.tsx)). Both cap output at ~1024px. **Requires a native dev-client rebuild** (`react-native-image-crop-picker` is a native module); web works with just `pnpm install`.
