...
| `RESEND_API_KEY` | dev (`villa-events`) | `sendAuthSignInEmail` (`functions/src/auth/`) | A **send-only** Resend API key, unrestricted (a `--domain-id`-restricted `sending_access` key was tried first but Resend returned a token that failed auth on every endpoint — file a support ticket before retrying that path). Sends the branded auth sign-in email from the verified `acceso.cultuvilla.es` domain. Server-side only — never shipped to clients. **Not yet created on beta/prod** — must create the domain in Resend, verify its DNS, and create this secret per-project before those functions deploy there. |
