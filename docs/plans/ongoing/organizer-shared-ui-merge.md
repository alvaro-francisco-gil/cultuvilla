...
- **Updated:** 2026-06-22
- **Stage:** ALL 8 phases implemented and **MERGED to `main`** (`86c0c2e`), after reconciling two rounds of main divergence (~35 commits incl. the pueblo-tab redesign, join-flow retirement, and barrio/place detail screens). **DEPLOYED to dev** (`villa-events`) on 2026-06-22 as `cultuvilla.app@gmail.com`. Reconciliation decisions: card-tap → main's detail screens; the section "+ Proponer"/"Gestionar" affordances → the shared propose/manage screens; kept main's `getOrgMemberCount` (getCountFromServer), `getBarrio`/`getPlace` getters, and redesigned village tab/tests.
- **Deploy (done — dev):** Rules+storage via `pnpm deploy:rules:dev` (place/barrio proposal gating, `isEventOrganizer`, `registrationContacts` subcollection). Functions deployed targeted — `firebase deploy --only functions:registerToEvent,functions:addWalkInRegistration` — because the full `deploy:functions:dev` aborts on an **orphaned `ogRenderer(europe-west1)` function** that exists in dev but has no source on `main` (added on an abandoned share-link-OG branch: `40f2acb`/`2b7354f`, never merged). `addWalkInRegistration` created, `registerToEvent` updated. **Cleanup TODO (operator decision):** `firebase functions:delete ogRenderer --region europe-west1 --project dev` to unblock future full-codebase function deploys.
