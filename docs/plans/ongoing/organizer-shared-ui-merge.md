...
- **Phase 3 note:** `OrganizationsManager` added (villager proposes peña/asociación → pending; organizer auto-approves via `requestOrganization`+`approveOrganization`). Org rules forbid member edit/withdraw, so proposers get no such affordance. Member-level `/village/[id]/organizations` now the shared manager; `/admin/organizations` is a wrapper. Tab routes everyone to the shared screen. **Pending-visibility (user verdict):** pending organizations are organizer-only — a villager sees a peña/asociación only once approved. Both the tab AND the `OrganizationsManager` list filter to `approved` for non-organizers; organizers load all statuses to approve/reject. This intentionally diverges from places/barrios, where pending IS visible to everyone (original brainstorming decision).
