...
- **EAS Submit profiles**: `internal` / `closed` / `production` map to the Play tracks `internal` / `alpha` / `production`. All three submit the **same** `production` build (package `com.cultuvilla.app`) — Play's closed-testing requirement is per package name, so a `com.cultuvilla.app.beta` build earns nothing toward it. Locked by [packages/shared/test/ci/storeRelease.test.ts](packages/shared/test/ci/storeRelease.test.ts).
...
- **Restore the review requirement the day the reviewer works here.** `ci.yml`'s `request-review` job is already wired and inert; it waits on the `SELF_HOSTED_REVIEW` repository variable, which waits on homelab's `ci-controller` config being applied to the host. When reviews start landing, set `requireApprovingReview: true` in `land.config.json` — leaving it false past that point keeps the weaker bar for nothing.
